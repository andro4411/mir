...
class ManageMenuController extends Controller
{
    public function manageMenu()
    {
        ...
        $data['headerMenus'] = ManageMenu::where(['menu_section'=> 'header','template_name' => $theme])->firstOrFail();
        $data['footerMenus'] = ManageMenu::where(['menu_section'=> 'footer' ,'template_name' => $theme])->firstOrFail();
    ...
    public function headerMenuItemStore(Request $request)
    {
        ...
        try {
            $theme = getTheme();
            $menu = ManageMenu::where('menu_section', 'header')
                ->where('template_name',$theme)->first();

            if (!$menu){

                $menu = new ManageMenu();
                $menu->menu_section = 'header';
                $menu->menu_items = $request->menu_item;
                $menu->template_name = $theme;
                $menu->save();
            }else{
    ...
    public function footerMenuItemStore(Request $request)
    {
        ...
        try {
            $theme = getTheme();
            $menu = ManageMenu::where(['menu_section' => 'footer','template_name' => $theme])->first();

            if (!$menu){
                $menu = new ManageMenu();
                $menu->menu_section = 'footer';
                $menu->menu_items = $request->menu_item;
                $menu->template_name = $theme;
                $menu->save();
            }else{
