...
class PaymentLogController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        return DataTables::of($deposit)
            ...
            ->addColumn('action', function ($item) {
                ...
                if (optional($item->gateway)->id > 999 && adminAccessRoute(config('role.payment.access.edit'))) {
            ...
            ->rawColumns(['no','name', 'method', 'amount', 'charge', 'payable', 'status', 'action'])->make(true);
    ...
    public function paymentRequest(Request $request)
    {
        ...
        return DataTables::of($funds)
            ...
            ->addColumn('action', function ($item) {
                ...
                $icon = $item->status == 2 ? 'pencil' : 'eye';
                if (adminAccessRoute(config('role.payment.access.edit'))) {
    ...
    public function action(Request $request, $id)
    {
        ...
        if ($request->status == '1') {
            ...
            $msg = [
                ...
                'charge' => currencyPosition($data->charge+0),
            ...
            $this->sendMailSms($data->user, 'PAYMENT_APPROVED',$msg);
    ...
    public function multipleApproved(Request $request)
    {
        if ($request->strIds == null) {
            ...
            Deposit::whereIn('id', $request->strIds)->get()->map(function ($data) {
                BasicService::preparePaymentUpgradation($data);
    ...
    {
        $deposits = Deposit::whereIn('id', $request->deposit_ids)->get()->map(function ($item) {
            $status = 'Unknown';
            if ($item->status == 0) {
            ...
            return [
                ...
                'Payable Amount' => fractionNumber(getAmount($item->payable_amount)) .' ' . $item->payment_method_currency,
                'Payable Amount in Base Currency' => currencyPosition($item->payable_amount_in_base_currency),
                'Base currency charge' => currencyPosition($item->base_currency_charge),
                'Status' => $status,
