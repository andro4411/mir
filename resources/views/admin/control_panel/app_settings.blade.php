@extends('admin.layouts.app')
@section('page_title', __('App Settings'))
@section('content')
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row align-items-end">
                <div class="col-sm mb-2 mb-sm-0">
                    <nav aria-label="@lang('breadcrumb')">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            <li class="breadcrumb-item">
                                <a class="breadcrumb-link" href="javascript:void(0)">@lang('Dashboard')
                                </a>
                            </li>
                            <li class="breadcrumb-item active" aria-current="page">@lang('Settings')</li>
                            <li class="breadcrumb-item active" aria-current="page">@lang('App Settings')</li>
                        </ol>
                    </nav>
                    <h1 class="page-header-title">@lang('App Settings')</h1>
                </div>
            </div>
        </div>

        <div class="row">
            <div class="col-lg-12">
                <div class="shadow p-3 mb-5 alert-soft-blue mb-4 mb-lg-7" role="alert">
                    <div class="alert-box d-flex flex-wrap align-items-center">
                        <div class="flex-shrink-0">
                            <img class="avatar avatar-xl"
                                 src="{{ asset('assets/admin/img/oc-megaphone-light.svg') }}"
                                 alt="@lang('Image Description')" data-hs-theme-appearance="default">
                            <img class="avatar avatar-xl"
                                 src="{{ asset('assets/admin/img/oc-megaphone-light.svg') }}"
                                 alt="@lang('Image Description')" data-hs-theme-appearance="dark">
                        </div>

                        <div class="flex-grow-1 ms-3">
                            <h3 class="alert-heading text-info mb-1">@lang("Attention!")</h3>
                            <div class="d-flex align-items-center">
                                <p class="mb-0 text-info"> @lang(" If you get 500(server error) for some reason, please turn on `Debug Log` and try again. Then you can see what was missing in your system. ")</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="col-lg-4">
                @include('admin.control_panel.components.sidebar', ['settings' => config('generalsettings.settings'), 'suffix' => 'Settings'])
            </div>
            <div class="col-lg-8" id="basic_control">
                <div class="d-grid gap-3 gap-lg-5">
                    <div class="card">
                        <div class="card-header">
                            <h2 class="card-title h4">@lang('App Settings')</h2>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('admin.app.settings.update') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="row mb-3">
                                    <div class="col-sm-6">
                                        <label for="siteTitleLabel" class="form-label">@lang('App Color')</label>
                                        <input type="color"
                                               class="form-control color-form-input @error('app_color') is-invalid @enderror"
                                               name="app_color"
                                               id="primaryColorLabel"
                                               placeholder="@lang('Primary Color')" aria-label="@lang('Primary Color')"
                                               value="{{ old('app_color',basicControl()->app_color) }}">
                                        @error('app_color')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-sm-6">
                                        <label for="AppVersionLabel"
                                               class="form-label">@lang('App Version')</label>
                                        <input type="text"
                                               class="form-control @error('cookie_button_name') is-invalid @enderror"
                                               name="app_version"
                                               id="AppVersionLabel" autocomplete="off"
                                               placeholder="@lang("App Version")" aria-label="@lang("App Version")"
                                               value="{{ old('app_version',$basicControl->app_version) }}">
                                        @error('app_version')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                                    </div>

                                </div>

                                <div class="row mb-3">
                                    <div class="col-sm-6">
                                        <label for="AppBuildLabel" class="form-label">@lang('App Build')</label>
                                        <input type="text"
                                               class="form-control  @error('app_build') is-invalid @enderror"
                                               name="app_build"
                                               id="AppBuildLabel" autocomplete="off"
                                               placeholder="@lang("App Build")" aria-label="@lang("App Build")"
                                               value="{{ old('app_build',$basicControl->app_build) }}">
                                        @error('app_build')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="d-flex mt-4">
                                            <div class="flex-grow-1">
                                                <div class="row align-items-center">
                                                    <div class="col">
                                                        <h5 class="mb-0">@lang('Is Major Version')</h5>
                                                        <span
                                                            class="d-block fs-6 text-body">@lang('Enable or Disable Is Major Version')</span>
                                                    </div>
                                                    <div class="col-auto">
                                                        <label class="row form-check form-switch mb-3" for="isMajorVersion">
                                                            <span class="col-4 col-sm-3 text-end">
                                                                <input type='hidden' value='0' name='is_major'>
                                                                <input
                                                                    class="form-check-input @error('is_major') is-invalid @enderror"
                                                                    type="checkbox" name="is_major" @checked($basicControl->is_major == 1)
                                                                    id="isMajorVersion"
                                                                    value="1">
                                                            </span>
                                                            @error('is_major ')
                                                            <span class="invalid-feedback d-block">{{ $message }}</span>
                                                            @enderror
                                                        </label>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                @if(adminAccessRoute(config('role.control_panel.access.edit')))
                                    <div class="d-flex justify-content-start">
                                        <button type="submit" class="btn btn-primary">@lang('Save changes')</button>
                                    </div>
                                @endif
                            </form>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>
@endsection

@push('css-lib')
    <link rel="stylesheet" href="{{ asset('assets/admin/css/tom-select.bootstrap5.css') }}">
@endpush
@push('js-lib')
    <script src="{{ asset('assets/admin/js/tom-select.complete.min.js') }}"></script>
    <script src="{{ asset("assets/admin/js/hs-file-attach.min.js") }}"></script>
@endpush

@push('script')
    <script>
        'use strict';
        (function () {
            HSCore.components.HSTomSelect.init('.js-select', {
                maxOptions: 500
            })
        })();

        $(document).ready(function () {

            new HSFileAttach('.js-file-attach-logo', {
                textTarget: "#logoImg"
            });


        })

    </script>
@endpush
